...
export function AgentHomeConsole({ agentName, permissions, history }: Props) {
  ...
  return (
    <main className="grid gap-4 py-1">
      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {cards.map((card) => {
          const disabled =
            ...
            card.remainingToday <= 0 ||
            card.remainingMonth <= 0 ||
            card.remainingTotal <= 0;
          ...
          return (
            <button
              ...
              className="grid gap-3 rounded-[24px] border border-[#d9cebf] bg-[rgba(248,244,237,0.96)] p-4 text-left transition hover:border-[#cabba9] hover:bg-[#fcf8f2] disabled:cursor-not-allowed disabled:opacity-55"
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="text-base font-semibold text-[#1f1a17]">{card.name}</div>
                  <div className="mt-1 text-xs text-[#74685b]">
                    {card.description || `点击后直接发码 · ${agentName}`}
                  </div>
                </div>
                <div className="rounded-full border border-[#dfd3c5] bg-[#f8f1e8] px-2.5 py-1 text-[11px] text-[#5f5347]">
      ...
      <section className="rounded-[24px] border border-[#ddd2c4] bg-[rgba(249,245,238,0.94)] p-4">
        <div className="mb-3 flex items-center justify-between gap-3">
          <div>
            <div className="section-label">最近发码</div>
            <div className="mt-1 text-sm text-[#5f5347]">点击任意记录可重新查看并复制内容。</div>
          </div>
        </div>
          ...
          <div className="grid gap-2">
            {orderedHistory.map((item) => (
              <button
                ...
                className="grid gap-1 rounded-[18px] border border-[#e1d6c8] bg-[#fbf7f1] px-3 py-3 text-left transition hover:bg-[#f7f1e8]"
              >
            ...
            ? (() => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-xs text-[#74685b]">
                      {copied ? "已复制到剪贴板" : "点击内容自动复制"}
                    </div>
                  </button>
                );
              })()
